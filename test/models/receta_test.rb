...
class RecetaTest < ActiveSupport::TestCase
  test "un plato exige porciones" do
    receta = Receta.new(nombre: "Milanesa", tipo: "plato")
    assert_not receta.valid?
    assert_includes receta.errors[:porciones], "es obligatorio en un plato"
  end
  ...
  test "suma el costo de sus ingredientes" do
    plato   = crear_plato(porciones: 4)
    ...
    assert_equal BigDecimal("297.5"), plato.costo_por_porcion
  end
  ...
  test "food cost, margen y precio sugerido" do
    plato  = crear_plato(porciones: 4, precio_venta: 1000)
    queso  = insumo_con_precio(precio: 10_000, cantidad: 1, unidad: "kg") # 10 $/g
    agregar(plato, queso, 100, "g")   # 1000 total -> 250 por porcion

    assert_equal BigDecimal(250), plato.costo_por_porcion
